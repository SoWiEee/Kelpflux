#!/usr/bin/env bash
# INTERFERENCE+BALANCE+FAIRNESS retrain — the bigger tail-fix lever. balance-only (see
...
COMMON=(--n-nodes 2 --gpus-per-node 1 --hetero-cluster --trace aimix
        ...
        --balance-coef "$BALANCE" --interference "$INTERFERENCE" --fairness-coef "$FAIRNESS")

train_one(){ local ARM="$1" SEED="$2" OUT="runs/aimix_fair_$1_s$2_${STAMP}"
...
log "### train aimix INTERFERENCE=$INTERFERENCE +BALANCE=$BALANCE +FAIRNESS=$FAIRNESS (mo): ${!ARM_FLAGS[*]} × seeds ${SEEDS[*]}  steps=$STEPS MAX=$MAX"
log "### COMMON: ${COMMON[*]}"

...
#   fcfs | backfill | score | SAC | RDSAC-mean | RDSAC-cvar | UXP-RL
...
#              → run_heavytail_ab with all ckpts → score + SAC + RDSAC-mean/-cvar + UXP-RL
...
#
#   bash eval/scripts/run_uxprl_hybrid6.sh
#   SEEDS="42 43" CONFIGS="learned" bash eval/scripts/run_uxprl_hybrid6.sh
set -uo pipefail
...
# ── local serve on :8003 (current code → can load UXP-RL checkpoints) ──
...
prewarm(){
  log "prewarming LLM on both GPU nodes ..."
  local ids="" j
...
common_args=(--serve-url "$SERVE" --login-pod "$LOGIN" --namespace $NS --controller-pod $CTL
  --family philly --n-jobs "${N_JOBS:-30}" --sigmas 1.0 --rounds "${ROUNDS:-3}" --warmup 1 --interleave
...
LEARNED = ["SAC", "RDSAC-mean", "RDSAC-cvar", "UXP-RL"]
ORDER = ["score", "SAC", "RDSAC-mean", "RDSAC-cvar", "UXP-RL", "backfill", "fcfs"]

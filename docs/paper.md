...
| **Hybrid** 高負載 | SAC | −9.7 | .255 | [−21.0,−0.9] | 否 |
| MDE≈12.6% | RDSAC-mean | −11.0 | .156 | [−18.7,−3.5] | 否 |
|  | RDSAC-cvar | −3.9 | .344 | [−10.7,+3.4] | 否 |
|  | CrossQ | −8.4 | .246 | [−15.4,−1.6] | 否 |
|  | RLPD | −11.9 | .115 | [−19.2,−5.0] | 否 |
|  | fcfs | −10.8 | **.030** | [−15.5,−6.2] | 否 |
|  | backfill | −4.9 | .246 | [−9.5,−1.0] | 否 |

三點結論。**(1) 多重比較校正移除了大部分「學習式較差」的顯著宣稱。** cuBLAS 場景校正後**無任何學習臂顯著**（校正前 seed 顯著的 CrossQ p=.011、RDSAC-cvar p=.033，Holm 校正後升至 .075／.200）；Hybrid 場景校正前顯著的 RDSAC-mean（.031）、RLPD（.019）校正後亦不再顯著（.156／.115）。故「學習式**顯著**較差」不成立，誠實降為「未穩健勝出」——這反而**強化**負結論的可信度。**(2) 唯一正面主張通過校正。** Hybrid 場景「score 勝 vanilla Slurm FCFS」（fcfs Holm p=.030）是全表**唯一**校正後仍顯著者，證實此正面觀察穩健。**(3) cuBLAS 的平坦是「證實的等價」而非檢力不足。** cuBLAS 場景 RDSAC-mean／CrossQ／fcfs／backfill 經 TOST **證實在 ±5% 內與 score 統計等價**（fcfs／backfill 的 90% CI 僅約 ±2%，等價極強），其 MDE≈4.9%、各臂效應正落此解析度邊界，故「策略空間平坦」是**證實的統計等價**、而非「樣本太少看不出差異」的假打平。Hybrid 場景則因變異大得多（pooled SD≈10.9%、MDE≈12.6%）、無臂達成 ±5% 等價。惟須強調 Hybrid 與 cuBLAS 性質不同：此場景各學習臂均值落在 −4～−12%（近或超出 ±5% 帶），**本就非「平坦／等價」而是「score 勝、學習式略差」的場景**，故「證實等價」並非此處的正確標的；殘留的問題是「學習式較差」是否**顯著**——n=8＋Holm 校正下除 fcfs 外均不顯著，更多 seed 能做的是**提升檢力以釐清此負向效應是否成立**（而非證實等價），列為未來工作。
...
- **多重比較與檢力（已處置）：** §5.3.1–5.3.3 每場景同時對 6–8 個臂檢定 score，已於 §5.3.4 補做 **Holm-Bonferroni 跨臂族校正**——校正後僅「score 勝 vanilla Slurm FCFS」穩健，各學習臂與 score 之差異均不再顯著，故本文不主張任何「學習式顯著較差」。同時以 **TOST 對 ±5% 帶檢定等價**、報告 **n=8 的 MDE**（cuBLAS≈4.9%、Hybrid≈12.6%）：cuBLAS 場景多臂證實 ±5% 等價（平坦為證實而非檢力不足）；Hybrid 為「score 勝、學習式略差」場景（均值 −4～−12%、非平坦），等價非其標的，n=8＋Holm 下除 fcfs 外差異不顯著，更多 seed 僅能釐清該負向效應是否顯著、無法證實等價（未來工作）。

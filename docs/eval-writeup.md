...
> **歷史文件說明。** 本報告為預-RDSAC-rewrite 的評估歷程紀錄，數字與部分結論已被 `docs/paper.md`（尤其 §5.8 排序致動結果）取代。倉庫清理時移除了下列僅供本歷史附錄使用的 harness 與 image recipe，其內文仍以原名引用之，**可由 git 歷史回溯**：`run_aiserve_live.py`、`eval_aiserve.py`、`baseline_switch.py`、`aggregate_baseline.py`、`run_live_5arm.sh`、`plot_paper_figures.py`、`sweep_stochastic.py`、`run_heavytail_ab.py`、`Dockerfile.htab2x1`。最終定案的評估 harness 見 `eval/scripts/{run_step3_prio.sh,scontrol_ab.py,aggregate_step3*.py,run_heavy150_aimix_5arm.sh,run_aimix6.sh}`。
...
- [!] **placement controller 端到端實機驗證（2026-07-04）——發現 actuation bug**：在 2×1（node-2 復活後）以 `sbatch -H` 端到端測 `placement_controller.py`：controller 正常 poll、`/act` 200（2 節點 obs=166 相符），但其 **slurmrestd `POST /job {required_nodes}`（v0.0.37）未被套用**——held 工作的 `ReqNodeList` 始終為 null、hold 未釋放，controller 卻在 `drain_and_apply` 迴圈裡對同一工作空轉（log「9 placed」實則 0 生效、單 cycle 卡 3 分鐘）。對照確認：**`scontrol update JobId=X ReqNodeList=<node>` 與 `sbatch -w <node>` 皆能正確釘節點**（held/pending 工作 ReqNodeList 立即設上）。故 re-pin 機制本身在 21.08 可行，壞的是 controller 走的 REST 欄位。**修法**：(a) 改用 slurmrestd 正確的節點欄位（查 v0.0.37 job-update schema，可能非 `required_nodes`），或 (b) 把顯式放置移到**提交時**——`job_submit.lua` 呼叫 `/act` 後設 `job_desc.req_nodes`（與 §4.2 eval 的 `sbatch -w` 同路徑、已驗證）。另注意 **node-2 會 flapping**（pod 重啟後短暫 IDLE→數分鐘後又 `DOWN+NOT_RESPONDING`，RealMemory=7000/FreeMem 極低），2-node 實機需先穩住 node-2。paper §3.2／§4.6 已據此改為「評估與生產皆以提交時節點綁定為準」。
  - **修法已實作(提交時 Lua,待部署)**:`chart/lua/rl_hook.lua` 新增 `RL_PLACEMENT`（opt-in）+ `RL_NODE_NAMES` → 解析 `/decide` 的 `node_j`、於提交時設 `job_desc.req_nodes`（Slurm honour 提交時 required nodes,與 §4.2 eval 的 `sbatch -w` 同路徑,fail-safe:未知 node_j／缺映射則跳過）。9 個 Lua 單元測試全過（`tests/lua/rl_hook_test.lua`）。**待你審閱後**:chart 渲染 `RL_NODE_NAMES`（GPU worker 節點名清單）+ `RL_PLACEMENT=true`、`lua.enabled=true`、`helm upgrade`。slurmrestd 事後釘節點路線放棄（v0.0.37 欄位不生效 + controller pod 無 scontrol/munge + JWT 認證機制複雜,不值得再挖）。
  - **2-node 完整實機評估(2026-07-04,node-2 穩定後)**:4 方(score/SAC/RDSAC-mean/RDSAC-cvar)× 3 train-seed × {σ0,σ1}、提交時 `-w`、drift-robust interleave、每格 n=60 pairs(`runs/live4_s{42,43,44}_*`、`live4_3seed_*_TABLES.md`)。**全學習臂輸 score**(mean ΔJCT σ0:cvar −10.3±5.0／mean −30.7±17.6／SAC −27.6±25.7;σ1:cvar −19.2／mean −19.0／SAC −16.8),per-seed t 檢定 p<0.01。**RDSAC-cvar 一貫最不差、尾端競爭**(某 seed σ0 Δp99 +6%)。node-2 全程 IDLE 無 flap。node-2 穩定法:重啟 3080 pod + `scontrol reconfigure`(根因=重啟後 slurmctld ping 舊 pod IP)。**注意:此為 sleep 工作負載(不建 CUDA context→異質性/MPS 量不到,只剩佇列效應),已被下方 real-CUDA+MPS 結果取代,不入 paper。**
  - [X] **★ 實機 real-CUDA + MPS 共置五方比較(2026-07-05)——結論反轉,學習式勝 score**(`runs/live5arm_s{42,43,44}_*`、`live5arm_*_TABLES.md`;harness `run_live_5arm.sh`,**local serve :8003** 因部署 image 舊、缺 CrossQ code)。把 sleep 換成**真 cuBLAS `gpu_workload` + MPS 分數共置**(Poisson、mps-oversub 1.0、桶 25/50/75/100),其餘不變(2×1、`-w`、interleave、3 seed、hetero ckpt、σ=1.0、n=78/seed),納 CrossQ 共五方。**RDSAC-mean 穩健勝 score**:各 seed ΔJCT +7.2/+12.3/+10.3%,seed-level one-sample t **ΔJCT +9.9±2.6% p=0.021、Δp99 +59.2±8.9% p=0.0074**;score p99 卡 83s(落慢卡/MPS 爆),rmean 壓到 25-39s。SAC +6.5%、CrossQ +5.1% 亦多勝,cvar −2.6%。**誠實:尾端勝利非中位數**——pooled 234 job 只 44% 更快(paired t p≈0.09、Wilcoxon p≈0.08),勝利集中在避開 score 的尾端災難。**機制**:sleep/exclusive 不觸及卡內共享槓桿→learned 輸;真實計算異質+MPS 共置才是 learned 有用的場景。已入 paper §4.2.1 表 4-1、摘要、§5(A 框架,領頭正面)。**鞏固 run(N_JOBS=40 ROUNDS=5、n=495)結果——修正框架**:平均勝利更強更穩(RDSAC-mean +15.2%,3 seed 全正 +5.2/+18.7/+21.7),**job-level 顯著**(pooled Wilcoxon **p<0.001**、log-JCT t p<0.001、61% job 更快;SAC +10%/57%、cvar +8.1%/55%、CrossQ +12.2%/55% 亦同向 p<0.001)。**但尾端 p99 翻轉不穩**:run1 +59%→run2 −116%,score 自身 p99 也 83→37s 擺盪→2-node 小叢集尾端由排程運氣主導,**不宣稱尾端優勢**。誠實界定:seed-level t(n=3)檢力不足 p≈0.10、pooled Wilcoxon 有 pseudoreplication 之虞;綜合=**方向極穩健(3 seed × 2 run 全正)、幅度 +10~15%、job-level 顯著**的平均勝利。paper §4.2.1 表 4-1/摘要/§5 已據此從「尾端勝利」改為「平均勝利」。這兌現了上方 [ ] item(line 611)的三手段(更多樣本 + 多 seed),把 §4.3 的 +3.9%(p=0.116)推進成穩健顯著的實機正面結果。
- [~] **CrossQ(Bhatt 2024)取代 RDSAC**(`dsac.py --crossq --utd-ratio 1`):BatchNorm 評論家 + **移除 target network**,current/next state 聯合前傳讓 BN 共享統計(cross trick)、stop-grad target、Adam β1=0.5。動機:RDSAC 的崩潰/高變異/auto-α 失穩全是 off-policy 分布式 critic + target-net 特有的;CrossQ 去掉這些來源、是 2024 的 SAC 後繼、計算省(去 target-net)、離散可用。**PPO<SAC 是連續控制結論,離散上不成立**,故不選 PPO。實作 `_CrossQCritic`/`_update_crossq`,6 CrossQ 測試 + 77 全套過,GPU 2×1(166-dim)smoke 乾淨、checkpoint 相容 serve/eval。**評估進行中**(sim 2×1、3 train-seed、σ∈{0,1}、配對 vs score,對照 SAC/RDSAC)。
- [ ] **SimbaV2 縮放架構**(future work,不確定收益):Simba/SimbaV2(KAIST 2024-2025)是**method-agnostic 的網路骨架**(輸入正規化+殘差+LayerNorm;V2 加 hyperspherical/weight norm),讓 deep RL 能穩定放大網路。可當作 CrossQ 的 critic 骨架變體對照,但屬架構而非演算法,收益不確定,列為後續。

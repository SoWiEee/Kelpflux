...
#### 4.3.1 精確測量：+3.9% 沒撐過,反轉成顯著的 −3.3%

上面的 +3.9% 不顯著（p=0.116），主因是 §4.3 跑在 slowdown_p99≈460 的**極端飽和**、JCT 變異巨大、且只有 n=96。為了「做到統計顯著」,按方法學降噪 + 加樣本重跑（**非飽和 regime**：poisson 到達 + `--mps-oversub 1.0`，slowdown_p99 從 460→**193**；**n 96→246**；聚焦 score vs RDSAC-mean）：

| arm | n | mean JCT | p99 | CVaR | slowdown_p99 | ΔJCT% | t-test p |
|---|--:|--:|--:|--:|--:|--:|--:|
| score | 246 | 167.9 | 425.3 | 373.3 | 192.8 | （baseline）| |
| RDSAC-mean | 246 | 173.5 | 427.2 | 377.0 | 193.3 | **−3.3** | **2.49e-12** |

**結果反轉**：一旦把變異砍掉（slowdown 460→193）+ 樣本三倍化，§4.3 的「名目 +3.9%」**翻成顯著的 −3.3%（RDSAC-mean 較慢、p=2.5e-12）**。所以那個 +3.9% 是**極端飽和 + 小樣本下的雜訊假象**,不是真實勝出。**精確測量下,item-1 的卡片異質感知在實機仍顯著略輸 score（−3.3%）**——與全文主軸（2×1 沒有 DRL arm 穩健贏 score）一致,並糾正了 §4.3 暫定的「名目小贏」讀法。

> 原始檔 `runs/htab_item1_20260622-103714/`（§4.3）、`runs/htab_item1_sig_20260622-183551/`（§4.3.1 顯著性重跑）

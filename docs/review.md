...
**(a) 表 5（n=246、job 層級）仍待處置。** 表 5（早期 BERT 放置四方比較）以 job 當獨立單位（偽重複、p 值 1e-12～1e-17 被灌水），與 §5.3.1「以正確分析層級——seed」自相矛盾；此表非 cuBLAS／hybrid 之列，未被階段一重分析涵蓋。建議：加註為「seed 層級方法定案前的初步量測」或移附錄，正式結論一律以 §5.3.1–5.3.4 的 seed 層級分析為準。
